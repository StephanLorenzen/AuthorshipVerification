...
def model(dinfo):
    ...
    char_pool8 = L.GlobalMaxPooling1D(name='char_pool8')
    word_pool3 = L.GlobalMaxPooling1D(name='word_pool3')
    word_pool6 = L.GlobalMaxPooling1D(name='word_pool6')
    ...
    for name in ['known', 'unknown']:
        ...
        c_out8 = char_pool8(char_conv8(char_embd(c_in)))
        w_out3 = word_pool3(word_conv3(word_embd(w_in)))
        w_out6 = word_pool6(word_conv6(word_embd(w_in)))
        p_out3 = pos_pool3(pos_conv3(pos_embd(p_in)))
        p_out6 = pos_pool6(pos_conv6(pos_embd(p_in)))

        concat = L.concatenate([c_out4,c_out8,w_out3,w_out6,p_out3,p_out6])
    ...
    model = Model(inputs=inls, outputs=[output], name='n7')
