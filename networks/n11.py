...
def model(dinfo):
    ...
    char_conv4 = L.Convolution1D(
            ...
            activation='relu',
            name='char_conv4')
    char_conv6 = L.Convolution1D(
            filters=500,
            kernel_size=6,
            strides=1,
            activation='relu',
            name='char_conv6')
    word_conv2 = L.Convolution1D(
            filters=500,
            kernel_size=2,
            strides=1,
            activation='relu',
    ...
    char_pool6 = L.GlobalMaxPooling1D(name='char_pool6')
    word_pool2 = L.GlobalMaxPooling1D(name='word_pool2')
    word_pool3 = L.GlobalMaxPooling1D(name='word_pool3')
    pos_pool3 = L.GlobalMaxPooling1D(name='pos_pool3')
    pos_pool5 = L.GlobalMaxPooling1D(name='pos_pool5')
    ...
    for name in ['known', 'unknown']:
        ...
        c_out6 = char_pool6(char_conv6(char_embd(c_in)))
        w_out2 = word_pool2(word_conv2(word_embd(w_in)))
        w_out3 = word_pool3(word_conv3(word_embd(w_in)))
        p_out3 = pos_pool3(pos_conv3(pos_embd(p_in)))
        p_out5 = pos_pool5(pos_conv5(pos_embd(p_in)))

        concat = L.concatenate([c_out4,c_out6,w_out2,w_out3,p_out3,p_out5])
        
        outls.append(concat)
   
    dist = L.Lambda(lambda x:absdiffsig(x[0],x[1]), output_shape=lambda in_shp: in_shp, name='distance')(outls)
